...
%      edge densities [5, 10, 15, 20] % (Rubinov & Sporns 2010).
%
...
for s = 1:numel(studies)
    ...
    % Print summary for the prespecified functional-network endpoint family.
    fprintf('\n  Density sweep summary (median delta across pairs):\n');
    fprintf('  %8s  %18s  %18s  %14s\n', ...
        'Density', 'Char. path length', 'Small-worldness', 'Modularity Q');
    for d = 1:nDens
        mask_pl = sweepT.target_density == densities(d) & ...
                  strcmp(sweepT.metric, 'Char. path length');
        mask_sw = sweepT.target_density == densities(d) & ...
                  strcmp(sweepT.metric, 'Small-worldness sigma');
        mask_mq = sweepT.target_density == densities(d) & ...
                  strcmp(sweepT.metric, 'Modularity Q');
        fprintf('  %8.0f%%  %+18.4f  %+18.4f  %+14.4f\n', ...
            densities(d)*100, median(sweepT.delta(mask_pl)), ...
            median(sweepT.delta(mask_sw)), median(sweepT.delta(mask_mq)));
    end
