...
%   percent change, exact Wilcoxon signed-rank p-value, supportive
%   bootstrap 95% CI + p-value, and Hedges' g_av effect size. This is the
%   single function
...
%   bootstrap then resamples clusters first, then
...
%   used. Bootstrap values are supportive uncertainty summaries; Wilcoxon
%   p-values are the primary paired inference for low-N well-level endpoints.
%
% INPUTS:
...
% See also: HEDGES_G_AV, PERMUTATION_TEST_EXACT, BH_FDR.
    ...
    % --- Wilcoxon signed-rank (primary low-N paired inference) -----------
    out.wilcoxon = signed_rank_exact(b, t, 'preferExact', true);
