...
%                          — plotted by src/figures/plot_correlograms.py
...
% See also: CONNECTIVITY_XCORR, PLOT_NETWORK_ON_MEA, RUN_FIGURES.
    ...
    layout = mea60_layout();
    mcsLabels = labels_for_channels(layout, opt.channels);
    ...
    export_correlogram(bSpikes, tSpikes, iGain, jGain, bMeta, tMeta, opt, ...
        ...
        sprintf('Largest delta edge (el. %d - %d)', ...
            mcsLabels(iGain), mcsLabels(jGain)), ...
    ...
    deadMask = detect_dead_channels(bRes.adjacency, layout, opt.channels, cfg.channels.reference);

    save_adj_heatmap(bRes.adjacency, labels.baseline, false, layout, opt.channels, deadMask, ...
        fullfile(heatDir, [prefix '_adj_baseline']));

    save_adj_heatmap(tRes.adjacency, labels.treatment, false, layout, opt.channels, deadMask, ...
        fullfile(heatDir, [prefix '_adj_treatment']));

    save_adj_heatmap(delta, '\Delta (treatment - baseline)', true, layout, opt.channels, deadMask, ...
    ...
    sidecar = struct( ...
        ...
        'strongest_baseline_edge_i',  mcsLabels(iTop), ...
        'strongest_baseline_edge_j',  mcsLabels(jTop), ...
        'strongest_baseline_weight',  bRes.adjacency(iTop, jTop), ...
        'largest_delta_edge_i',       mcsLabels(iGain), ...
        'largest_delta_edge_j',       mcsLabels(jGain), ...
...
function labels = labels_for_channels(layout, channels)
    [ok, idx] = ismember(channels(:).', layout.channelList);
    if ~all(ok)
        error('fig_connectivity_exemplar:UnknownChannel', ...
            'Requested channel is not present in MEA layout.');
    end
    labels = layout.mcsLabels(idx);
end
...
%EXPORT_CORRELOGRAM Write correlogram data as CSV + JSON for Python.
    ...
    meta = struct( ...
        ...
        'channel_i_mcs',   labels_for_channels(layout, opt.channels(ci)), ...
        'channel_j_mcs',   labels_for_channels(layout, opt.channels(cj)), ...
        'max_lag_ms',      opt.maxLagMs);
...
% -------------------------------------------------------------------------
function deadMask = detect_dead_channels(A, layout, channels, refChannel)
...
%   non-contributing hardware faults.
    ...
    deadMask(channels == refChannel) = true;
    ...
    excluded = labels_for_channels(layout, channels(deadMask));
    if any(deadMask)
...
% -------------------------------------------------------------------------
function save_adj_heatmap(A, ttl, ~, layout, channels, deadMask, outBase)
...
%   Produces Inkscape-ready PDFs via src/figures/plot_heatmaps.py.

    mcsLabels = labels_for_channels(layout, channels);
    [sortedLabels, sortOrder] = sort(mcsLabels);
