...
%
%   This is the filtered-trace panel for the DOI and ketanserin rate figures.
%   The matching raster panel is produced by fig_rate_panel_raster.m.
%
%   Options (Name-Value):
...
%                     '<study>_rate_panel_A_traces')
    ...
    % Panel sized for direct manuscript assembly.
    ...
    for i = 1:nCh
        ...
        plot_trace(ax, bTraces{i}, bFs, [zoomStart zoomEnd], 1,               yLim, style);

        ax = axes('Position', [xB2 y wFull rowH]); %#ok<LAXES>
        plot_trace(ax, bTraces{i}, bFs, [0 bDur],             opt.decimFactor, yLim, style);

        ax = axes('Position', [xT1 y wZoom rowH]); %#ok<LAXES>
        plot_trace(ax, tTraces{i}, tFs, [zoomStart zoomEnd], 1,               yLim, style);

        ax = axes('Position', [xT2 y wFull rowH]); %#ok<LAXES>
        plot_trace(ax, tTraces{i}, tFs, [0 tDur],             opt.decimFactor, yLim, style);
        ...
        annotation(fig, 'line', [scaleX scaleX], [y + rowH*0.25, y + rowH*0.75], ...
            'Color', 'k', 'LineWidth', style.scaleBarLineWidth);
        annotation(fig, 'textbox', [scaleX + 0.006, y + rowH*0.15, 0.09, rowH*0.7], ...
            ...
            'FontSize', style.tickFontSize, 'FontWeight', 'bold', ...
...
% =========================================================================
function plot_trace(ax, traceVolts, fs, tRangeSec, decim, yLimUv, style)
    ...
    plot(ax, t, double(seg) * 1e6, 'k', 'LineWidth', style.traceLineWidth);
    xlim(ax, tRangeSec);
    ylim(ax, yLimUv);
    axis(ax, 'off');
end
