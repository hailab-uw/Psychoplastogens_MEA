...
Usage:
    uv run python src/figures/plot_correlograms.py             # all
    uv run python src/figures/plot_correlograms.py --study doi # DOI only
"""
...
def plot_correlogram(csv_path: Path, meta_path: Path):
    ...
    ax.plot(lags, baseline, color=bc, linewidth=1.2, label=meta["baseline_label"])
    ax.plot(lags, treatment, color=tc, linewidth=1.2, label=meta["treatment_label"])
    ax.axvline(0, color=(0.6, 0.6, 0.6), linestyle=":", linewidth=0.6)

    ax.set_xlabel("Lag (ms)", fontsize=LABEL_SIZE, fontfamily=FONT_FAMILY)
    ax.set_ylabel("Correlation (z)", fontsize=LABEL_SIZE, fontfamily=FONT_FAMILY)
    ax.set_title(
        meta["title"], fontsize=TITLE_SIZE, fontweight="bold", fontfamily=FONT_FAMILY
    )
...
def process_study(study: str):
    ...
    for csv_path in csv_files:
        meta_path = csv_path.with_name(csv_path.name.replace("_data.csv", "_meta.json"))
...
def main():
    ...
    parser.add_argument(
        "--study", choices=["doi", "ket"], help="Process one study (default: both)"
    )
