...
Usage:
    uv run python src/figures/plot_heatmaps.py             # all
    uv run python src/figures/plot_heatmaps.py --study doi # DOI only
"""
...
def plot_heatmap(csv_path: Path, meta_path: Path):
    ...
    ax.set_xlabel("Electrode (MCS)", fontsize=LABEL_SIZE, fontfamily=FONT_FAMILY)
    ax.set_ylabel("Electrode (MCS)", fontsize=LABEL_SIZE, fontfamily=FONT_FAMILY)
