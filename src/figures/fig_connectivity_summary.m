...
% See also: RUN_CONNECTIVITY, NETWORK_METRICS, PAIRED_STATS, RUN_FIGURES.
    ...
    topologyFamily = ismember({stats.metric}, ...
        {'meanShortestPath', 'smallWorldnessSigma', 'modularity'});

    % The confirmatory topology family keeps nonredundant endpoints:
    % integration, small-world balance, and community segregation.
    pFamilyWilcoxon = arrayfun(@(s) s.summary.wilcoxon.p, stats(topologyFamily));
    [pWilcoxonAdj, wilcoxonRejected] = bh_fdr(pFamilyWilcoxon, 0.05);

    % Bootstrap FDR is retained as a supportive/SI sensitivity layer.
    pFamilyBootstrap = arrayfun(@(s) s.summary.bootstrap.pDelta, stats(topologyFamily));
    [pBootstrapAdj, bootstrapRejected] = bh_fdr(pFamilyBootstrap, 0.05);

    familyIdx = find(topologyFamily);
    familyPosition = zeros(size(topologyFamily));
    familyPosition(familyIdx) = 1:numel(familyIdx);
    for m = 1:numel(stats)
        stats(m).summary.wilcoxon.inTopologyFamily = topologyFamily(m);
        stats(m).summary.wilcoxon.pBHFDR           = NaN;
        stats(m).summary.wilcoxon.significantBH    = false;
        stats(m).summary.bootstrap.pDeltaBHFDR     = NaN;
        stats(m).summary.bootstrap.significantBH    = false;
        famPos = familyPosition(m);
        if famPos > 0
            stats(m).summary.wilcoxon.pBHFDR        = pWilcoxonAdj(famPos);
            stats(m).summary.wilcoxon.significantBH = wilcoxonRejected(famPos);
            stats(m).summary.bootstrap.pDeltaBHFDR  = pBootstrapAdj(famPos);
            stats(m).summary.bootstrap.significantBH = bootstrapRejected(famPos);
        end
        stats(m).summary.wilcoxon.significantRaw  = ...
            stats(m).summary.wilcoxon.p < 0.05;
    end
    ...
    idxPL  = strcmp({stats.metric}, 'meanShortestPath');
    idxMod = strcmp({stats.metric}, 'modularity');
    idxSW  = strcmp({stats.metric}, 'smallWorldnessSigma');
    ...
    % === Panel 3: paired small-worldness ==================================
    save_paired_panel(stats(idxSW), labels, colors, ...
        fullfile(panelDir, [study '_paired_small_worldness']));

    % === Panel 4: forest plot =============================================
    save_forest_panel(stats, colors, ...
        fullfile(panelDir, [study '_forest_plot']));

    % === Panel 5: spatial decay ===========================================
    ...
    fprintf('fig_connectivity_summary(%s): 5 panels saved to %s\n', study, panelDir);

    % --- Numeric sidecars -------------------------------------------------
    metricTable = struct('metric', {}, 'label', {}, ...
        'median_baseline', {}, 'median_treatment', {}, ...
        'median_delta', {}, 'ci_delta_lo', {}, 'ci_delta_hi', {}, ...
        'p_bootstrap', {}, 'p_bootstrap_bh', {}, ...
        'p_wilcoxon', {}, 'p_wilcoxon_bh', {}, ...
        'p_bh_fdr', {}, 'hedges_g_av', {});
    for m = 1:numel(stats)
        s = stats(m).summary;
        row = struct();
        ...
        if isfield(s.bootstrap, 'pDeltaBHFDR')
            row.p_bootstrap_bh = s.bootstrap.pDeltaBHFDR;
        else
            row.p_bootstrap_bh = NaN;
        end
        row.p_wilcoxon        = s.wilcoxon.p;
        if isfield(s.wilcoxon, 'pBHFDR')
            row.p_wilcoxon_bh = s.wilcoxon.pBHFDR;
            row.p_bh_fdr      = s.wilcoxon.pBHFDR;
        else
            row.p_wilcoxon_bh = NaN;
            row.p_bh_fdr      = NaN;
        end
...
%SAVE_PAIRED_PANEL  Standalone paired dot-plot with IQR shading.
    ...
    yBracket = yMax * 1.08;
    [pDisplay, pLabel, sigDisplay] = display_wilcoxon_p(stat.summary.wilcoxon);
    if sigDisplay; star = '*'; else; star = 'n.s.'; end
    plot(ax, [1 2], [yBracket yBracket], 'k-', 'LineWidth', 0.8);
    text(ax, 1.5, yBracket * 1.01, star, ...
        'HorizontalAlignment', 'center', 'FontSize', 13, ...
        'FontName', 'Arial', 'FontWeight', 'bold');

    if pDisplay < 0.001; pStr = sprintf('%s < .001', pLabel);
    else;                pStr = sprintf('%s = %.3f', pLabel, pDisplay); end
...
%SAVE_FOREST_PANEL  Standalone horizontal forest plot of effect sizes.
    ...
    for m = 1:nMetrics
        y   = yPos(m);
        g   = gVals(m);
        sigDisplay = isfield(statsArr(m).summary.wilcoxon, 'pBHFDR') && ...
                     isfinite(statsArr(m).summary.wilcoxon.pBHFDR) && ...
                     statsArr(m).summary.wilcoxon.pBHFDR < 0.05;
    ...
    legend(ax, [h1 h2], {'BH p < .05', 'Not BH sig.'}, ...
...
function [pDisplay, pLabel, significant] = display_wilcoxon_p(wilcoxon)
%DISPLAY_WILCOXON_P  Prefer family-adjusted p-values for plotted inference.
    if isfield(wilcoxon, 'pBHFDR') && isfinite(wilcoxon.pBHFDR)
        pDisplay = wilcoxon.pBHFDR;
        pLabel = 'BH p';
    else
        pDisplay = wilcoxon.p;
        pLabel = 'Wilcoxon p';
    end
    significant = isfinite(pDisplay) && pDisplay < 0.05;
end
